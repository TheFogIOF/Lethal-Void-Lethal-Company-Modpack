...
Each thing conversion entry has 6 parts; the name of the thing to convert, and a list for each conversion setting corresponding to the name of the thing (or things for a chance) to convert to.
...
If the input or output has a "!" before it (e.g. "!masked") it will represent an enemy conversion (only indoor enemies are officially supported; YMMV for nonstandard modded enemies/outdoor enemies)
...
Names are automatically converted to lowercase when read, matching is based on the item data name (the same text that appears when you scan an item). Enemy names are based on the enemy name in the enemy type data (you'll have to check the wiki (or ask the dev for modded enemies) for this value)
...
3. In the square braces, add entries like the above example. Every entry except the last requires a comma at the end.
